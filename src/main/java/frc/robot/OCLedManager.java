...
public class OCLedManager {
     ...
     */
    private static void progressBar(int hue, double percentage){
    ...
    private static void hotspot(int initOffset, int speed, int hue, int sat){
        ...
        for (var i=0;i<buffer.getLength();i++){
            ...
            if(value>waveThresholdValue) buffer.setHSV(i, hue, sat, value);
        }
    }
    //-----
    ...
    private static void slideshow(){// cycle through all patterns every 6 seconds
        int patternIndex = (int)((Timer.getFPGATimestamp()/6)%(Pattern.values().length-1));
    ...
    private static void tide(int hue, int satMin){
        for(int i=0;i<buffer.getLength();i++){
            int currSat = (int)(workingSat+(i*(255-satMin)/buffer.getLength())) % 256;
            buffer.setHSV(i, hue, currSat+satMin, 255);
        }
        workingSat = (int)(Timer.getFPGATimestamp()*80) % 256;
    }

    private static void rockingWave(int hueRange, int hueInitial){
        for(var i=0;i<buffer.getLength();i++){
            int currHue = (int)(((Math.sin(workingHue/180*Math.PI)+1) * (hueRange/2.0) + (i*hueRange/buffer.getLength())) % hueRange);
